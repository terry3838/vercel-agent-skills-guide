...
**Vercel Agent Skills 관련 핵심 용어 정리**

> 현재 upstream은 `react-view-transitions`를 포함한 **7개 스킬 축**으로 읽는 편이 맞다.
...
- **핵심 포인트**: 단순 문서가 아니라, 작업 맥락에 따라 자동 발화되는 지식 팩이라는 점이 중요하다.
...
Anthropic이 만든 AI 코딩 에이전트 CLI. Agent Skills를 설치해 확장할 수 있는 대표 호스트 중 하나다.
...
Vercel 배포 스킬에 포함된 배포 스크립트. Vercel CLI 인증 없이 프로젝트를 배포할 때 쓰는 no-auth fallback 흐름과 연결된다.

- **핵심 의미**: 대화형 환경에서도 배포를 바로 수행하게 해 주는 보조 실행 표면
...
Vercel 배포 스킬에서 Vercel CLI 인증이 불가능한 환경에서 사용하는 배포 방법. 인증 없이도 Preview URL과 Claim URL을 반환한다.
...
Vercel에서 프로덕션이 아닌 배포에 생성되는 임시 URL. 배포 결과를 즉시 검증하는 가장 중요한 출력 중 하나다.
...
React의 View Transition API 및 Next.js 연동을 다루는 최신 스킬 축. 페이지 전환, shared element transition, directional animation 같은 화면 전환 경험을 다룬다.

현재 upstream에서 최근 갱신이 가장 활발한 영역 중 하나다.
