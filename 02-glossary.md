...
AI 코딩 에이전트(Claude Code, Cursor, Copilot 등)에 주입하는 **전문 지식 패키지**. SKILL.md 파일을 중심으로 구성되며, 에이전트가 특정 도메인의 모범 사례를 자동으로 적용하도록 만든다.
...
스킬의 **컴파일된 전체 가이드** 파일. 여러 규칙 파일(`rules/*.md`)을 하나로 합친 결과물. AI 에이전트가 스킬의 모든 규칙을 한 번에 참조할 수 있도록 제공된다.

- **생성 방법**: `packages/react-best-practices-build`의 빌드 도구가 자동 생성
...
여러 서브컴포넌트가 공유 Context를 통해 상태를 공유하는 React 패턴. Boolean prop 남용을 피하는 핵심 아키텍처.
...
`state`, `actions`, `meta`를 명확히 구분한 React Context 타입 정의. 소비자가 어떤 값을 쓸 수 있는지 명확하게 한다.
...
`deploy-to-vercel` 스킬에 포함된 배포 스크립트. Vercel CLI 인증 없이도 프로젝트를 배포할 수 있다 (no-auth fallback).
...
React에서 값이나 컴포넌트를 캐싱해 불필요한 재계산/리렌더를 방지하는 기법. `useMemo`, `useCallback`, `React.memo`가 대표적.
...
스크롤 이벤트에 `{ passive: true }` 옵션을 추가해 브라우저가 스크롤 중 `preventDefault()`가 호출되지 않음을 알게 하는 기법. 스크롤 성능을 크게 개선한다.
...
React 19+의 서버 컴포넌트에서 같은 요청 내 함수 결과를 캐싱하는 API. 여러 컴포넌트가 같은 데이터를 요청할 때 중복 요청을 방지한다.
...
Agent Skill의 핵심 파일. 에이전트가 스킬을 어떻게 사용할지 정의한다. frontmatter에 `name`, `description` 메타데이터를 포함하며, 에이전트가 언제 이 스킬을 활성화할지 판단하는 트리거 역할을 한다.
...
독립적으로 실행할 수 있는 비동기 작업이 직렬로 실행되는 패턴. 총 실행 시간이 N배로 늘어난다.
